...
export default function UserUsagePage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card><CardHeader className="pb-3 border-b"><div className="flex items-center justify-between"><CardTitle className="text-lg flex items-center gap-2"><Clock className="h-5 w-5 text-primary" /> Detailed User Breakdown</CardTitle><Badge variant="outline" className="font-bold">{aggregatedUsage.length} Staff Identities</Badge></div><CardDescription>Individual time tracking based on 10-minute heartbeat frequency.</CardDescription></CardHeader><CardContent className="p-0">
          {isLoading ? (<div className="flex justify-center py-20"><RefreshCw className="h-8 w-8 animate-spin text-primary" /></div>) : aggregatedUsage.length === 0 ? (<div className="text-center py-20 text-muted-foreground italic">No activity logs found for the selected period.</div>) : (
            <Table><TableHeader className="bg-muted/30"><TableRow><TableHead className="pl-6">User / Identity</TableHead><TableHead>Branch</TableHead><TableHead>Role</TableHead><TableHead>Last Active</TableHead><TableHead className="text-right pr-6">Est. Usage Time</TableHead></TableRow></TableHeader>
              <TableBody>{aggregatedUsage.map((u) => (<TableRow key={u.userId} className="hover:bg-muted/20"><TableCell className="pl-6"><div className="flex items-center gap-3"><div className="h-8 w-8 rounded-full bg-primary/10 text-primary flex items-center justify-center text-xs font-bold">{u.userName.charAt(0)}</div><span className="font-bold text-sm">{u.userName}</span></div></TableCell><TableCell><Badge variant="outline" className="text-[10px] uppercase font-bold flex w-fit items-center gap-1"><MapPin className="h-3 w-3" /> {u.branch}</Badge></TableCell><TableCell><span className="text-[10px] font-black uppercase text-muted-foreground tracking-tighter">{u.role}</span></TableCell><TableCell className="text-xs text-muted-foreground font-medium">{u.lastActive ? format(u.lastActive, 'dd/MM HH:mm') : 'N/A'}</TableCell><TableCell className="text-right pr-6"><Badge variant="secondary" className={`font-black text-xs px-3 py-1 border-primary/10 ${u.heartbeats > 0 ? 'bg-primary/5 text-primary' : 'bg-muted text-muted-foreground opacity-50'}`}>{formatUsageTime(u.heartbeats)}</Badge></TableCell></TableRow>))}</TableBody></Table>
          )}
        </CardContent></Card>
    </div>
  );
