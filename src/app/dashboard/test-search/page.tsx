...
export default function TestSearchPage() {
  ...
  }, [students, selectedBranch, dateRange, testType, isAdmin, profile, isFromBranch]);
...
function StudentProfileViewContent({ student, db }: any) {
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        <section className="space-y-4">
          ...
          <div className="grid gap-4 text-sm">
            <ProfileItem icon={<Clock className="h-3 w-3" />} label="Admission Date" value={toUI(student.registrationDate)} />
            <ProfileItem icon={<Calendar className="h-3 w-3" />} label="Date of Birth" value={toUI(student.dob)} />
            <ProfileItem icon={<Phone className="h-3 w-3" />} label="Mobile" value={student.phone} />
            <ProfileItem icon={<Fingerprint className="h-3 w-3" />} label="Aadhar" value={student.aadharNo} />
            <ProfileItem icon={<FileText className="h-3 w-3" />} label="Online App ID" value={student.onlineAppNo} />
            <Separator className="col-span-full my-1 opacity-50" />
            <div className="grid grid-cols-2 gap-4 col-span-full">
              <ProfileItem icon={<BookOpen className="h-3 w-3" />} label="Learners No" value={student.learnersNo} />
              <ProfileItem icon={<Calendar className="h-3 w-3" />} label="Learners Date" value={toUI(student.learnersDate)} />
            </div>
            <div className="grid grid-cols-2 gap-4 col-span-full">
              <ProfileItem icon={<Car className="h-3 w-3" />} label="DL No" value={student.drivingNo} />
              <ProfileItem icon={<Calendar className="h-3 w-3" />} label="DL Test Date" value={toUI(student.testDate)} />
            </div>
            <Separator className="col-span-full my-1 opacity-50" />
            <ProfileItem icon={<MapPin className="h-3 w-3" />} label="Address" value={student.address} fullWidth />
          </div>
        </section>
        <section className="space-y-4">
          ...
          <div className="space-y-2">
            {student.courses?.map((c: string, i: number) => (
              <div key={i} className="p-3 rounded-lg border bg-muted/20 flex justify-between items-center">
                <span className="font-medium text-sm">{c === 'Others' ? (student.specialCourseName || 'Custom') : c}</span>
      ...
      <Separator />
      <section className="space-y-4">
        <h3 className="font-bold flex items-center gap-2 text-primary border-b pb-2"><Calendar className="h-4 w-4" /> Recent Attendance</h3>
        {isAttendanceLoading ? (
              ...
              <TableBody>
                {[...attendance].sort((a, b) => (b.date || '').localeCompare(a.date || '')).slice(0, 5).map((a: any) => (
                  <TableRow key={a.id} className="hover:bg-muted/30">
                    ...
                    <TableCell><Badge variant="outline" className="text-[9px] uppercase font-bold">{a.type}</Badge></TableCell>
                    <TableCell className="text-right font-bold text-xs text-primary">{a.duration}h</TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
  ...
  return (
    <Card className={colorMap[color]}>
      <CardHeader className="p-4 pb-2">
        <CardTitle className="text-[10px] font-black uppercase flex items-center gap-2 tracking-widest opacity-70">
          {icon} {label}
        </CardTitle>
      </CardHeader>
      <CardContent className="p-4 pt-0">
        ...
        {breakdown && Object.keys(breakdown).length > 0 && (
          <div className="mt-2 flex flex-wrap gap-1">
            {Object.entries(breakdown).map(([type, hours]: [string, any]) => hours > 0 && (
              <Badge key={type} variant="outline" className="text-[9px] px-1.5 py-0 h-5 font-mono font-bold bg-muted/30 border-primary/10">
